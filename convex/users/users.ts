...
export async function getUserProfileByUsernameHandler(ctx: Ctx, username: string) {
    const profile = await ctx.db
        .query("userProfiles")
...
export async function createUserProfileHandler(ctx: MutationCtx, identity: UserIdentity): Promise<Id<"userProfiles">> {
  // Generate a unique username
  const baseUsername = identity.preferredUsername || 
...
export async function updateUserProfileHandler(ctx: MutationCtx, identity: UserIdentity): Promise<Id<"userProfiles">> {
...
export async function upsertUserProfileHandler(ctx: MutationCtx): Promise<Id<"userProfiles">> {
    const identity = await ctx.auth.getUserIdentity();
    if (!identity) {
        throw new Error("User not authenticated");
    }

    const existingProfile = await getUserProfileByUserIdHandler(ctx, identity.subject);
...
export async function deleteUserProfileHandler(ctx: MutationCtx): Promise<void> {
    const { profile } = await requireUserIdentity(ctx);
