  ...
  const sharedCategory = await ctx.db
    ...
    .withIndex('by_campaign_name', (q) =>
      q
        ...
        .eq('name', SYSTEM_TAG_CATEGORY_NAMES.Shared.toLowerCase()),
    )
    .unique()
  if (!sharedCategory) {
...
export async function getSharedAllTag(
  ...
  const sharedTags = await ctx.db
    ...
    .collect()
  const sharedAllTag = sharedTags.find(
    (t) => t.name === SYSTEM_TAG_CATEGORY_NAMES.Shared.toLowerCase(),
  )
  if (!sharedAllTag) {
...
export async function ensureAllPlayerSharedTags(
  ...
  const existingMemberIds = new Set(
    existing.filter((t) => !!t.memberId).map((t) => t.memberId),
  )
