...
export const updateCampaign = mutation({
  ...
  handler: async (ctx, args): Promise<Id<"campaigns">> => {
    const { identityWithProfile, campaignWithMembership } = await requireCampaignMembership(ctx, { campaignId: args.campaignId },
...
export const updateCampaignMemberStatus = mutation({
  ...
  handler: async (ctx, args): Promise<Id<"campaignMembers">> => {
    ...
    const callerAsDm = callerMembership.find(
      (m) => m.campaignId === member.campaignId && m.role === CAMPAIGN_MEMBER_ROLE.DM,
    );
    if (!callerAsDm) {
    ...
    if (args.status === CAMPAIGN_MEMBER_STATUS.Accepted && member.status !== CAMPAIGN_MEMBER_STATUS.Accepted) {
      await ctx.db.patch(member.campaignId, {
        playerCount: Math.max(0, campaign.playerCount + 1),
        updatedAt: now,
      });
    } else if (args.status === CAMPAIGN_MEMBER_STATUS.Removed && member.status === CAMPAIGN_MEMBER_STATUS.Accepted) {
      await ctx.db.patch(member.campaignId, {
        playerCount: Math.max(0, campaign.playerCount - 1),
        updatedAt: now,
      });
