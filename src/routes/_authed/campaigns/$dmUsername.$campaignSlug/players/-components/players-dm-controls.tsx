...
export default function PlayersDmControls({ onOpenRequests, onCopyJoinUrl }: PlayersDmControlsProps) {
  ...
  return (
    <div className="mt-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <CopyField className="bg-slate-100" text={joinUrl} placeholder="Join URL not configured" onCopy={onCopyJoinUrl} />
        <div className="relative inline-flex w-fit shrink-0 self-start md:self-auto">
          <Button variant="outline" disabled={campaignWithMembership.status === "pending" || campaignMember?.role !== CAMPAIGN_MEMBER_ROLE.DM} onClick={onOpenRequests}>Manage Requests</Button>
          {pendingCount > 0 && (
