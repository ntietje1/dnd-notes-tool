...
export default function ShareSideMenuButton({ block, freezeMenu, unfreezeMenu }: ShareSideMenuButtonProps) {
  ...
  const items: { key: string; name?: string; username?: string; tag: Tag; applied: boolean }[] = useMemo(() => {
    const list: { key: string; name?: string; username?: string; tag: Tag; applied: boolean }[] = [];
    if (sharedAllTag) {
      list.push({
        ...
        {items.map((it) => (
          ...
          >
            <span className="flex min-w-0 flex-col leading-tight">
              <span className="truncate font-medium" title={(it.name || (it.username ? `@${it.username}` : "Player"))}>
                {it.name ? it.name : (it.username ? `@${it.username}` : "Player")}
              </span>
              {it.name && it.username && (
                <span className="truncate text-xs text-muted-foreground" title={`@${it.username}`}>
                  @{it.username}
                </span>
              )}
            </span>
