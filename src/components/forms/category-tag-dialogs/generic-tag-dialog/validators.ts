...
export function validateTagDescription(value: string, maxLength: number, singular: string): string | undefined {
  const v = value.trim();
  if (!v) return undefined;
  if (v.length > maxLength) return `${singular} description must be ${maxLength} characters or fewer`;
  return undefined;
}

export function validateTagName(value: string, maxLength: number, singular: string): string | undefined {
  const v = value.trim();
  if (!v) return `${singular} name is required`;
  if (v.length > maxLength) return `${singular} name must be ${maxLength} characters or fewer`;
  return undefined;
}
